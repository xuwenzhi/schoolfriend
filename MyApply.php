...
require_once 'include/smarty/Smarty.class.php';
	...
	//查询出数据库中招聘信息的个数
	$sql_all_apply_count = "Select count(ApplyId) From t_apply Where SFUserId =".$_SESSION['USERID'];
	//建立查询 获取当前页面的数据
	$aa = ($assPage->pageNow-1)*$assPage->pageSize;//每一页的起始位置
	$bb = $aa+$assPage->pageSize;
	$sql_pagenow_apply = "Select ApplyUnit,ApplyId, ApplyTime From t_apply where SFUserId =".$_SESSION['USERID']." And ApplyETime>now() order by ApplyId desc Limit $aa, $bb ";//建立该查询
	//执行改查询
	$sqlHelper->excute_dql_asspage($sql_all_apply_count, $sql_pagenow_apply, $assPage);
	...
	//分配信息至模板
	$smarty->assign("QZXX" ,$assPage->pageArr); //分配招聘信息
