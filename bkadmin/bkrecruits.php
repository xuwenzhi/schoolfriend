<?php
	...
	include_once '../include/ComFunction.php';
		...
		//查询出招聘应聘信息的个数
		$sql_count = "Select count(RecruitId) from t_recruit ";
		//下面建立查询当前页中显示数据
		$aa = $assPage->pageNow*$assPage->pageSize;
		$pageCount = $assPage->pageNow*$assPage->pageSize;
		$temp_page = $assPage->pageNow*$assPage->pageSize;
		$sql_Recruit_pageNow= "Select * from t_recruit order by RecruitOrder desc,RecruitId desc limit $temp_page , $assPage->pageSize";
		$sqlHelper->excute_dql_asspage($sql_count, $sql_Recruit_pageNow, $assPage);
		function getRecruitList(){
			...
			for($i=0; $i<count($assPage->pageArr); $i++){
				...
				echo "<input type='hidden' name='recruit_id' value=".$assPage->pageArr[$i]['RecruitId']." >";
				echo "</form>";
				echo "</td>";
					
				echo "<td align=left>".$assPage->pageArr[$i]['RecruitTitle']."</td>";
				echo "<td align='center'>".getUserFromT_class($assPage->pageArr[$i]['SFUserId'])."</td>";
				echo "<td align='center'>".date("Y-m-d",strtotime($assPage->pageArr[$i]['RecruitTime']))."</td>";
				echo "<td align='center'><a href='dispose_recruits.php?recruits_id=".$assPage->pageArr[$i]['RecruitId']."&type=4'><font color='red'>修改/删除</font></a></td>";
				echo "</tr>";
			}
		}
		...
		function getPage(){
			global $assPage;
			echo "<a href='bkrecruit.php?pageNow=0'>首页</a>　";
			if($assPage->pageNow>0){
				$prePage = $_GET['pageNow']-1;
				echo "<a href='bkrecruit.php?pageNow=".$prePage."'>上一页</a>　";
			}else{
				echo "上一页　";
			}
			...
			for($i=0; $i<10; $i++){
				...
				if($now_page <= $assPage->pageCount-1){
					echo "<a href='bkrecruit.php?pageNow=".$now_page."'>[".$temp_page."]</a>　";
				}
			}
			if($assPage->pageNow<$assPage->pageCount-1){
				...
				echo "<a href='bkrecruit.php?pageNow=".$nextPage."'>下一页　</a>";
			}else{
				echo "　下一页　";
			}
		if($assPage->pageCount-1 >=0){
			...
			echo "<a href=bkrecruit.php?pageNow=$temp_page_last>末页</a>　";
        ...
        <table width="100%" border="0" cellspacing="1" cellpadding="1"  bgcolor="#CBD8E1">
          ...
          <!-- 这里是进行添加和管理新闻公告的链接  START -->
        	  ...
        	  <!-- 这里 添加新闻的地方  addWords.php?type=1  这里希望能够做活了 最好一个页面添加任何信息都可以    -->
    		    <td>　　<a href='addWords.php?type=4'><font color='red' size='4'><b>添加信息</b></font></a>　　　　　　　　 <a href="bkrecruits.php"><font color='red' size='4'><b>管理信息</b></font></a></td>
    		  </tr>
