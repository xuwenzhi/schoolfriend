<?php
	...
	//查询出数据库中新闻公告的个数
	$sql_all_product_count = "Select count(ProductId) From t_product";
	//建立查询 获取当前页面的数据
	$aa = ($assPage->pageNow-1)*$assPage->pageSize;//每一页的起始位置
	$bb = $aa+$assPage->pageSize;
	$sql_pagenow_products = "Select ProductId, ProductName, ProductRTime From t_product where ProductETime>now() and Visibility=1 order by ProductOrder desc,ProductId desc Limit $aa, $bb ";//建立该查询
	//执行改查询
	$sqlHelper->excute_dql_asspage($sql_all_product_count, $sql_pagenow_products, $assPage);
	...
	//分配信息至模板
	$smarty->assign("CPTJ" ,$assPage->pageArr); //分配新闻公告
