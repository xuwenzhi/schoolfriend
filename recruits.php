<?php
	...
	//查询出数据库中招聘信息的个数
	$sql_all_recruit_count = "Select count(RecruitId) From t_recruit";
	//建立查询 获取当前页面的数据
	$aa = ($assPage->pageNow-1)*$assPage->pageSize;//每一页的起始位置
	$bb = $aa+$assPage->pageSize;
	$sql_pagenow_recruits = "Select RecruitId, RecruitTitle, RecruitTime From t_recruit where RecruitETime>now() order by RecruitOrder desc,RecruitId desc Limit $aa, $bb ";//建立该查询
	//执行改查询
